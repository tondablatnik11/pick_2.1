...
def get_shift(time_val):
    ...
    try:
        # Převedení na sekundy od půlnoci pro snazší porovnání
        t_str = str(time_val).strip()
        if len(t_str) == 8: # Format HH:MM:SS
            h, m, s = map(int, t_str.split(':'))
        elif len(t_str) == 6: # Format HHMMSS
...
def render_daily_kpi(df_pick, raw_vekp):
    ...
    if df_pick is not None and not df_pick.empty:
        ...
        if date_col in df_p.columns and time_col in df_p.columns:
            ...
            if not pick_daily.empty:
                ...
                pick_daily['Category'] = pick_daily.get('Queue', 'Pick')
 ...
 # --- 3. VÝSLEDKY A PRODUKTIVITA ---
    ...
    if pbi_rows:
        ...
        # Filtrujeme jen validní časy (mínus jedna znamená neznámý čas)
